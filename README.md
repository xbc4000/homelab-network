...
Complete configuration for a MikroTik RB3011UiAS home lab router running RouterOS 7.22.
...
| AP2 | wAP2nD-1 | 2.4GHz WiFi, ch11, CAPsMAN CAP |
| Server1 | Dell PER730XD | VLAN10 |
| Server2 | Dell PER630 | VLAN20 |

## Features

- **6 isolated VLANs** with strict inter-VLAN firewall rules
...
- **Back to Home VPN** — remote access via MikroTik app (WireGuard, port 17723)
- **DDoS protection** — SYN/UDP flood detection, connection limits, per-VLAN rate limiting, whitelist
...
- **Syslog** forwarding to Pi-hole (topics: info, firewall)
- **Encrypted backups** — daily RSC export, weekly AES-SHA256 binary backup
- **Interface graphing** — pppoe-wan, vlan10, vlan20, vlan30, vlan40, vlan60
- **SNMP** for Grafana/mktxp metrics
...
rb3011-homelab/
├── config/
│   └── rb3011-config.rsc       # Complete router configuration (live export)
├── aps/
...
│   ├── services.md             # Pi-hole, Back to Home, Dude, container config, backups
...
│   ├── pihole-restart.rsc      # Restart Pi-hole container
│   ├── security-lockdown.rsc   # Apply security lockdown (run from VLAN10 only)
│   └── syslog-setup.rsc        # Enable syslog forwarding to Pi-hole
└── README.md
```
...
| Back to Home VPN | see /ip cloud print on router |
...
- RouterOS version: **7.22** (latest stable as of 2026-03-21)
- Firmware: **7.22** (current = upgrade, nothing to do)

## AP Setup

Both APs use import files for reliable configuration. See `aps/` directory.
Import files handle: bridge setup, static IP, NTP, timezone, service hardening, 
firewall removal, MAC server lockdown, CAP mode — all in one shot without 
mid-import disconnection issues.
...
Tested on RouterOS **7.22** (stable, released 2026-03-09). All configuration uses RouterOS 7 syntax.
Notable differences from v6 documented in [docs/troubleshooting.md](docs/troubleshooting.md).
